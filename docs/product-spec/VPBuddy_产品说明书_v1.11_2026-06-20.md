...
>
> **后续口径校准(2026-06-24)**:
> 1. **架构形态**: VPBuddy 是客户端-服务端架构，不是纯本地单机。桌面客户端采集音频并展示结果，服务端负责 ASR、结构化提取、文档生成、Demo 生成等重计算。
> 2. **实时性**: 结构化事实、6 类文档和 Demo 都是会议中实时生成、实时展示的，不是会议结束后才有初稿。
> 3. **AI 主动边界**: AI 可以主动在客户端内展示结论、建议、风险判断和候选方案；禁止的是主动外发给客户、主动触发投屏等对外动作。
> 4. **ASR 实现**: 实际实现为服务端 funasr paraformer-zh，客户端上传音频流，不再依赖会议平台原生 ASR。
>
> 本文档其余内容保持 v1.11 原貌，以上校准点如与正文冲突，以校准为准。
