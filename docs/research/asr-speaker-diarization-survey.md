...
**结论摘要**: ⚠️ **2026-06-21 ADR-0008 Superseded** — 本调研 4 平台对比中飞书妙记作为"首选推荐",**已删除 by ADR-0008**。当前真实架构(ADR-0004) = **VP 桌面客户端麦克风/系统音频 loopback + 服务端 faster-whisper large-v3 + pyannote-audio 3.1**(本地模型,不依赖任何会议平台 SDK)。原 4 平台对比表保留作历史(2026-06-20 调研数据),但结论不再适用。详见 `docs/decisions/0008-ADR-0001-决策1-Superseded.md`。
...
### 1. ~~飞书妙记(首选推荐 ⭐⭐⭐⭐⭐)~~ → **Superseded by ADR-0008 (2026-06-21)**

**2026-06-21 ADR-0008 后**:飞书妙记不再作为数据源(详见 ADR-0008)。本节作为 2026-06-20 调研历史保留,所有"✅"特性现已不适用于 VPBuddy。

~~**核心优势**:~~
- ~~基础版 300 分钟/月免费(企业版无限)~~
...
### v1.13 候选(本次调研支持)
- ~~**首选飞书妙记**:零成本 + 说话人自动关联昵称 + API 充足~~ → **Superseded by ADR-0008 (2026-06-21)**;真实架构(ADR-0004)用 VP 桌面客户端麦克风 + Whisper + pyannote
...
interface TranscriptSegment {
  ...
  speaker_name?: string;       // ~~平台显示名(飞书/腾讯自动关联~~ → ADR-0008 Superseded,人工/stt_map 填入)
  text: string;                // 转写文本
