...
// =============================================================================
// Windows WASAPI Loopback (v0.22.1 — raw COM via windows 0.52, AUDCLNT_STREAMFLAGS_LOOPBACK)
// =============================================================================
...
mod wasapi_loopback {
    ...
    pub fn create_loopback() -> Result<(mpsc::Receiver<Vec<i16>>, u32, WindowsLoopback)> {
        ...
        let enumerator: IMMDeviceEnumerator =
            unsafe { MMDeviceEnumerator::new() }.context("创建 MMDeviceEnumerator 失败")?;
        ...
        let client: IAudioClient = client_unk.cast()?;

        let mut pwfx: *mut WAVEFORMATEX = std::ptr::null_mut();
        unsafe {
            client.GetMixFormat(&mut pwfx).context("GetMixFormat 失败")?;
        }
        ...
        let capture_client: IAudioCaptureClient = cap_unk.cast()?;
        ...
        std::thread::spawn(move || {
            while !stop_clone.load(Ordering::SeqCst) {
                ...
                if unsafe { capture_client.GetNextPacketSize(&mut packet_len) }.is_err() {
                ...
                while packet_len > 0 {
                    ...
                    if flags == 0 && !data.is_null() && num_frames > 0 {
                        ...
                        let samples: Vec<i16> = if bits == 32 {
                            let floats = unsafe { std::slice::from_raw_parts(data as *const f32, nf * ch) };
                            downmix_f32_to_i16(floats, ch)
                        } else if bits == 16 {
                            let ints = unsafe { std::slice::from_raw_parts(data as *const i16, nf * ch) };
                            downmix_i16(ints, ch)
                        } else {
                            Vec::new()
                        };
                    ...
                    unsafe { let _ = capture_client.ReleaseBuffer(num_frames); }
                    if unsafe { capture_client.GetNextPacketSize(&mut packet_len) }.is_err() {
                        packet_len = 0;
                    }
                }
        ...
        let guard = WindowsLoopback { _client: client };
        Ok((rx, sample_rate, guard))
    }

    fn downmix_f32_to_i16(floats: &[f32], ch: usize) -> Vec<i16> {
        match ch {
            1 => floats.iter().map(|&s| (s.clamp(-1.0f32, 1.0f32) * 32767.0f32) as i16).collect(),
            2 => floats.chunks(2).map(|c| (((c[0] + c[1]) * 0.5f32).clamp(-1.0f32, 1.0f32) * 32767.0f32) as i16).collect(),
            n => floats.chunks(n).map(|c| {
                let v = c.iter().sum::<f32>() / n as f32;
                (v.clamp(-1.0f32, 1.0f32) * 32767.0f32) as i16
            }).collect(),
        }
    }

    fn downmix_i16(ints: &[i16], ch: usize) -> Vec<i16> {
        match ch {
            1 => ints.to_vec(),
            2 => ints.chunks(2).map(|c| ((c[0] as i32 + c[1] as i32) / 2) as i16).collect(),
            n => ints.chunks(n).map(|c| {
                (c.iter().map(|&s| s as i32).sum::<i32>() / n as i32) as i16
            }).collect(),
        }
    }
