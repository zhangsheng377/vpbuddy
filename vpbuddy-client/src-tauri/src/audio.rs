...
// =============================================================================
// Windows WASAPI Loopback (v0.22.2 — raw COM with manual GUID/WAVEFORMATEX, windows 0.52)
// =============================================================================
...
mod wasapi_loopback {
    ...
    const IID_IAUDIO_CLIENT: GUID = GUID::from_values(
        0x1CB9AD4C, 0xDBFA, 0x4C32, [0xB1, 0x78, 0xC2, 0xF5, 0x68, 0xA7, 0x03, 0xB2],
    );
    const IID_IAUDIO_CAPTURE_CLIENT: GUID = GUID::from_values(
    ...
    pub fn create_loopback() -> Result<(mpsc::Receiver<Vec<i16>>, u32, WindowsLoopback)> {
        ...
        let wfx = WAVEFORMATEX {
            ...
            nBlockAlign: channels as u16 * (bits_per_sample as u16 / 8),
                ...
                while packet_len > 0 {
                    ...
                    if flags == 0 && !data.is_null() && num_frames > 0 {
                        ...
                        let samples: Vec<i16> = if bits == 32 {
                            downmix_f32(unsafe { std::slice::from_raw_parts(data as *const f32, nf * ch) }, ch)
                        } else if bits == 16 {
                            downmix_i16(unsafe { std::slice::from_raw_parts(data as *const i16, nf * ch) }, ch)
                        } else {
                            Vec::new()
                        };
                        if !samples.is_empty() {
        ...
        Ok((rx, sample_rate, WindowsLoopback { _client: client }))
    }

    fn downmix_f32(floats: &[f32], ch: usize) -> Vec<i16> {
        match ch {
            ...
            n => floats.chunks(n).map(|c| {
                ((c.iter().sum::<f32>() / n as f32).clamp(-1.0f32, 1.0f32) * 32767.0f32) as i16
            }).collect(),
        }
    }
