...
mod wasapi_loopback {
    ...
    pub fn create_loopback() -> Result<(mpsc::Receiver<Vec<i16>>, u32, WindowsLoopback)> {
        ...
        let enumerator: IMMDeviceEnumerator = unsafe {
            CoCreateInstance(
                &windows::Win32::Media::Audio::MMDeviceEnumerator::IID,
                None,
                CLSCTX_ALL,
            )
        ...
        let (tx, rx) = mpsc::sync_channel::<Vec<i16>>(64);
        let capturing = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(true));
        let capturing_clone = std::sync::Arc::clone(&capturing);
                        ...
                        && frames > 0 && !data.is_null()
                    {
                        ...
                        let mono: Vec<i16> = if channels == 1 {
                            ...
                            f32s.chunks(channels as usize)
                                .map(|f| {
                                    (f.iter().sum::<f32>() / channels as f32 * 32767.0) as i16
                                })
                                .collect()
                        };
